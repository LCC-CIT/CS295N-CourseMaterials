<h1>Week 7 Overview for Winter 2026</h1>

**CS 295N, Web Dev 1: ASP.NET**

| Weekly Topics                       |                                                   |
| ----------------------------------- | ------------------------------------------------- |
| 1. Intro to web dev                 | 6. EF and database                                |
| 2. Intro to MVC, deploying to Azure | <mark>7. LINQ, seed data</mark>                   |
| 3. Managing data: forms and models  | 8. Input validation                               |
| 4. Unit testing                     | 9. Authentication & Authorization                 |
| 5. Midterm quiz, start term project | 10. Debugging, term Project, project presentation |
| 11. Final quiz                      |                                                   |



<h2>Contents</h2>

[TOC]

## Announcements

- I don't have any announcements. Do any of you have something you want to announce?


## Due Dates

- Lab 5 beta version PR: Sunday, 2/15
- Lab 5 code review: Tuesday, 2/17
- Lab 5 production version: Thursday: 2/19
- Lab 6 beta version PR: Sunday, 2/22



## Discussion

- Was the video showing how to finsihe setting up your app to use a database helpful?
- Any questions on lab 5?



## Overview of Week 7

This week you will:

- Add seed data to your web app.
- Add a search feature using LINQ (Language INtegrated Query).
- Publish your site with it's database to Azure



---

[![Creative Commons License](https://i.creativecommons.org/l/by/4.0/80x15.png)](http://creativecommons.org/licenses/by/4.0/) These ASP.NET Core MVC course materials written by [Brian Bird](https://profbird.dev) in <time>2026</time>  are licensed under a [Creative Commons Attribution 4.0 International License](http://creativecommons.org/licenses/by/4.0/). 

---

